...
public class VaultController {
    ...
    public ResponseEntity<?> sendShareEmail(@Valid @RequestBody ShareEmailRequest shareEmailRequest) {
        try {
            otpService.sendShareNotificationEmail(shareEmailRequest);
            return ResponseEntity.ok(new ApiResponse(true, "Encrypted share link sent to " + shareEmailRequest.getRecipientEmail()));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ApiResponse(false, e.getMessage()));
        }
    }
