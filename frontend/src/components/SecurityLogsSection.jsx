  ...
  return (
    <div className="flex flex-col gap-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {/* Total Logins */}
        <div className="glass-panel p-4 border-indigo-500/20 bg-indigo-500/[0.03] flex flex-col justify-between">
          ...
          <div className="text-2xl font-extrabold text-indigo-400 font-mono mt-2 flex items-baseline justify-between">
            <span>{totalLogins}</span>
            <span className="text-[10px] font-semibold text-slate-400">Events</span>
          </div>
        </div>

        {/* Successful Logins */}
        <div className="glass-panel p-4 border-emerald-500/20 bg-emerald-500/[0.03] flex flex-col justify-between">
          <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5" /> Successful Logins
          </span>
          <div className="text-2xl font-extrabold text-emerald-400 font-mono mt-2 flex items-baseline justify-between">
        ...
        {/* Failed Logins */}
        <div className="glass-panel p-4 border-rose-500/20 bg-rose-500/[0.03] flex flex-col justify-between">
          ...
          <div className="text-2xl font-extrabold text-rose-400 font-mono mt-2 flex items-baseline justify-between">
        ...
        <div className="glass-panel p-4 border-amber-500/20 bg-amber-500/[0.03] flex flex-col justify-between">
          <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
            <AlertTriangle className="w-3.5 h-3.5" /> Suspicious Events
          </span>
          <div className="text-2xl font-extrabold text-amber-400 font-mono mt-2 flex items-baseline justify-between">
        ...
        {/* Security Alerts */}
        <div className="glass-panel p-4 border-rose-500/30 bg-rose-500/[0.04] flex flex-col justify-between">
          <span className="text-[10px] font-bold text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div className="flex border-b border-white/10 gap-2 flex-wrap">
        ...
        >
          ...
          <span>Analytics Overview & Visuals</span>
        </button>
                ...
                {unifiedRecentActivities.map((act) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-[11px] text-slate-400 font-mono shrink-0 self-end sm:self-center">
                        {act.user && (
                          <span className="flex items-center gap-1 text-slate-400">
                            <User className="w-3 h-3 text-indigo-400" /> {act.user}
                          </span>
                        )}
                        <span className="flex items-center gap-1 text-slate-400">
                          <Clock className="w-3 h-3 text-indigo-400" /> {formatDate(act.timestamp)}
                        </span>
                      </div>
                    </div>
                  );
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-300 border-collapse">
                <thead>
                  <tr className="border-b border-white/10 bg-slate-950/80 text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {suspicious.map((item) => (
                    <tr key={item.id} className="hover:bg-white/[0.02] transition">
                      ...
                      <td className="py-3 px-4 font-medium text-slate-300">{item.description}</td>
                      <td className="py-3 px-4 font-mono text-indigo-300">{item.email}</td>
                      <td className="py-3 px-4 font-mono text-[11px] text-slate-400">{formatDate(item.detectedAt)}</td>
                  ...
                  {auditLogs.map((log) => {
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-white/[0.02] transition">
                        ...
                        <td className="py-3 px-4 font-medium text-slate-200">{log.description}</td>
                        <td className="py-3 px-4 font-mono text-[11px] text-slate-400">{log.email}</td>
                        <td className="py-3 px-4 font-mono text-[11px] text-slate-400 text-right">{formatDate(log.timestamp)}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {activeSubTab === 'login_logs' && (
        <div className="glass-panel border-white/10 bg-slate-900/50 overflow-hidden flex flex-col">
          <div className="p-4 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-indigo-400" />
              <h3 className="font-bold text-xs text-white uppercase tracking-wider">Login Activity Log Table</h3>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[10px] text-slate-400 font-mono">Showing {loginLogs.length} entries</span>
              {loginLogs.length > 0 && (
                <button
                  type="button"
                  onClick={handleClearLogs}
                  disabled={clearing}
                  className="text-xs py-1.5 px-3 rounded-xl border border-rose-500/30 bg-rose-500/10 text-rose-300 hover:bg-rose-500/20 hover:border-rose-500/50 flex items-center gap-1.5 transition disabled:opacity-50 font-semibold"
                >
                  ...
                  {loginLogs.map((log) => {
                    ...
                    return (
                      <tr key={log.id || log.slNo} className="hover:bg-white/[0.02] transition">
                        <td className="py-3 px-4 font-mono text-slate-400 font-semibold">{log.slNo}</td>
                        ...
                        <td className="py-3 px-4 font-medium text-slate-200">{log.activity}</td>
                        <td className="py-3 px-4 font-mono text-[11px] text-slate-300">{displayDate}</td>
                        <td className="py-3 px-4 text-right">
