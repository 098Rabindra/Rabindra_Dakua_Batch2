...
export default function ShareCredentialModal({ 
  ...
  const activeItem = vaultItems.find((i) => i.id === selectedItemId) || item || (vaultItems.length > 0 ? vaultItems[0] : null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-lg animate-fadeIn">
      {/* Large Screen Modal Container (max-w-3xl) */}
      <div className="glass-panel w-full max-w-3xl overflow-hidden border-indigo-500/20 shadow-2xl bg-slate-950/95 flex flex-col max-h-[92vh]">
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6">
          ...
          <div className="p-5 rounded-2xl bg-slate-900/80 border border-white/[0.06] flex flex-col gap-4 shadow-lg">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              {/* Dropdown Select for All Credentials */}
              {vaultItems.length > 0 && (
          ...
          <form onSubmit={handleSendShareEmail} className="p-4 rounded-xl bg-slate-900/40 border border-white/[0.04] flex flex-col gap-3">
            ...
            {emailSentSuccess && (
              <div className="p-2.5 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-200 text-xs font-bold flex items-center gap-2">
                ...
                <span>Encrypted sharing notification sent to {recipientEmail}!</span>
              </div>
            )}
