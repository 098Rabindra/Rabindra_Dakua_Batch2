...
export default function SessionTimeoutModal({ isOpen, secondsLeft, onStayLoggedIn, onLogoutNow }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-lg animate-fade-in">
      <div className="glass-panel w-full max-w-md overflow-hidden border-rose-500/30 shadow-2xl bg-slate-950/90 relative">
        ...
        <div className="p-6 flex flex-col items-center text-center gap-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              For security, your password vault session will automatically lock and log out when the timer reaches 0. Move mouse or click below to extend session.
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 w-full mt-2">
            <button
              onClick={onLogoutNow}
              className="btn-secondary text-xs uppercase font-bold tracking-wider py-2.5 px-4 flex items-center gap-1.5 text-rose-400 hover:bg-rose-500/10 border-rose-500/20"
            >
