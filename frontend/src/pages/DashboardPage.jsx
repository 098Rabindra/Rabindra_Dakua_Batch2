...
export default function DashboardPage() {
  ...
  const handleUnlockSubmit = async (e) => {
    ...
    try {
      ...
      if (user?.email) {
        try {
          await authApi.login({ email: user.email, password: masterPassInput });
        } catch (apiErr) {
      ...
      if (vaultItems.length > 0) {
        const sampleItem = vaultItems.find((i) => i.encryptedPassword && i.iv);
        if (sampleItem) {
          const testDec = await decryptPassword(sampleItem.encryptedPassword, sampleItem.iv, derivedKey);
          if (testDec.includes('Invalid Master Password') || testDec.includes('Decryption Error')) {
            setUnlockError('Incorrect Master Password. Unable to decrypt vault.');
            setUnlocking(false);
            return;
          }
        }
        ...
        <main className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto">
          ...
          {!isVaultUnlocked && (
            <div className="glass-panel p-5 border-amber-500/20 bg-amber-500/[0.02] flex flex-col gap-3 mb-2 shadow-premium">
              ...
              {unlockError && (
                <div className="flex items-center gap-2 text-xs text-rose-400 font-semibold bg-rose-500/10 px-3 py-2 rounded-lg border border-rose-500/20 animate-fadeIn">
